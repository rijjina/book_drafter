...
ไฟล์ร่างและไฟล์แก้ทั้งหมดเป็น Markdown โดย `revised.md` มีลำดับก่อน
`draft.md` ระบบสร้าง DOCX เฉพาะ `final/manuscript.docx` หลัง final QC เป็น
...
7. **Markdown-first และ quality-first**
   การร่างและแก้ใช้ Markdown เท่านั้น ไม่สร้าง DOCX/PDF ระหว่างทาง และไม่ตัด
   เกณฑ์ ข้อยกเว้น คะแนน blocker หรือ page mapping เพื่อจำกัด token
...
ตำแหน่ง global Skill ของ Antigravity IDE คือ

```text
$HOME\.gemini\config\skills\write-thai-academic-book
```

หากต้องการติดตั้งเป็น Antigravity plugin:
...
ตำแหน่งติดตั้งเริ่มต้นของ Antigravity CLI plugin คือ

```text
$HOME\.gemini\antigravity-cli\plugins\write-thai-academic-book
```
...
./scripts/install.sh antigravity-plugin
...
### ขั้นที่ 8: ผลิต DOCX ฉบับสุดท้าย

ก่อนเรียก task นี้ ให้บันทึก final approval เป็น `Status: APPROVED` และ
`Deliverable: DOCX`
...
ระบบต้องไม่เขียนทับ `source/original-manuscript.docx` แต่สร้างไฟล์ Markdown
สำหรับบทที่แก้ เช่น

```text
chapters/chapter-NN/revised.md
chapters/chapter-NN/revision.md
...
| `revise-chapter <NN>` | แก้บทตาม QC | `revised.md`, `revision.md` | ใช่ |
...
| `revise-manuscript` | แก้ต้นฉบับนำเข้าตาม QC | `revised.md`, `revision.md`, `revision-log.md` | ใช่ |
| `final-qc` | ตรวจทั้งเล่มก่อนผลิตไฟล์ | `final-qc.md`, `preflight-report.md` | ใช่ |
| `produce-document` | ผลิตไฟล์สุดท้ายหลังยืนยัน DOCX | `manuscript.docx` | ใช่ |
...
- `final/approval.md` ระบุ `Deliverable: DOCX`
